...
window.addEventListener("load", async () => {
  ...
  try {
    ...
    registerNumber.value = result.data?.reg_number == 0 ? "" : result.data?.reg_number;
    address.value = result.data?.address;
    result.data?.photo != ""
      ? (profilePicture.src = 'http://localhost:8080/MINI%20PROJECTS/server/liveFile.php?data=' + result.data?.photo)
